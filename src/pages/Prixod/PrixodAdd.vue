<template>
    <div class="prixod">
        <div class="prixod_head">
            ...
            <div class="exit">
                <n-icon size="40" color="red" @click="exit" style="cursor: pointer"><CloseSquareFilled/></n-icon>
            </div>
        </div>
        <div class="prixod_content">
            <n-form style="display: grid; grid-template-columns: 0.1fr 0.3fr 0.25fr 0.1fr 0.25fr; align-items: center; gap: 15px;" ref="formRef" :model="prixod" :rules="rules">
                <n-form-item :label="t('data')">
                    <n-date-picker v-model:value="prixod.datetime" type="date" size="large" disabled/>
                </n-form-item>
                <n-form-item :label="t('kontragent')" path="kontragent_id">
                    <n-select
                        :options="poatavshikList"
                        filterable
                        label-field="name"
                        value-field="id"
                        :placeholder="t('kontragent')"
                        v-model:value="prixod.kontragent_id"
                        size="large"
                        @search="searchKontragent"
                    >
                        <template #empty>
                        <div>
                            ...
                            <n-button dashed type="primary" @click="pastavshikModal(), (showChange = 'pastavshik')" >Қўшиш</n-button>
                        </div>
                        </template>
                    </n-select>
                </n-form-item>
                            ...
                            >
                                <template #empty>
                                    <div>
                                        ...
                                        <div style="height: 10px;"></div>
                                        <n-button dashed type="primary" @click="maxsulotModal(), (showChange = 'maxsulot')" >Қўшиш</n-button>
                                    </div>
                                </template>
                            </n-select>
                        </td>
                        <td><n-input-number v-model:value="item.price" size="large" :placeholder="t('price')" :show-button="false" :parse="useParsenumber" :format="useFormatnumber"/></td>
                        <td><n-input-number v-model:value="item.count" size="large" :placeholder="t('count')" @update:value="yigindi(item)" :show-button="false" :parse="useParsenumber" :format="useFormatnumber"/></td>
                        <td><n-input-number v-model:value="item.summa" size="large" :placeholder="t('total_summa')" :show-button="false" readonly :parse="useParsenumber" :format="useFormatnumber"/></td>
                        <th style="width: 50px; padding: 5px;">
                            <div class="delete" @click="deleteRow(item, index)"><n-icon size="25" color="red"><DeleteFilled/></n-icon></div>
                        </th>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="prixod_btn">
            <div class="button">
                ...
                <n-button type="success" size="large" style="width: 205px;" @click="saveDoc" :loading="loading">
                    <n-icon size="20"><SaveAltRound/> </n-icon> 
                    {{ $t('save_btn') }}
                </n-button>
            </div>
        </div>
...
const pastavshikModal = () => {
    AddModal.value = true
}
const maxsulotModal = () => {
    AddModal.value = true
}
const searchKontragent = (e) => {
    values.value = e
}
...
const addInfo = (e) => {
    ...
    if(e == 'pastavshik'){
        if(values.value){
            axios.post('/kontragent/create', data)
            .then(function (res) {
                if(res.success){
                    loadBar.value = false;
                    
                    getAllKontragent()
                    prixod.value.kontragent_id = res.data.id
                    message.success('Паставшик яратилди')
...
const saveDoc = async () => {
    try {
        ...
        if(prixod.value.prixod_table.length > 0 && prixod.value.prixod_table[0].product_id) {
            ...
            let method;
            if (route.query.id) {
                method = axios.patch('/prixod/update/'+route.query.id, data)
            }else{
                method = axios.post('/prixod/create', data)
            }
...
const getOnePrixod = () => {
    if (route.query.id) {
        axios.get('/prixod/info/'+route.query.id)
        .then(function (res) {
            ...
            for(let i = 0; i < res.data.prixod_tables.length; i++){
                res.data.prixod_tables[i].price = Number(res.data.prixod_tables[i].price)
                res.data.prixod_tables[i].count = Number(res.data.prixod_tables[i].count)
                res.data.prixod_tables[i].summa = Number(res.data.prixod_tables[i].summa)

                prixod.value.prixod_table.push(res.data.prixod_tables[i])
            }
        })
        .catch(function (error) {
            console.log(error.message);
