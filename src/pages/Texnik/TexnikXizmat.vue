<template>
    <div class="container">
        ...
        <div class="content">
            <n-data-table :pagination="paginationReactive" :columns="columns" :data="tekshiruvList"
                :row-props="rowProps" striped />
        </div>
    </div>

    <n-modal v-model:show="addedModal" class="custom-card" preset="card" :style="{ width: '1000px' }" :title="texnikId ? t('update') : t('add_btn')">
        <TexnikCreate @close="closeModal" :texnikId="texnikId" />
    </n-modal>
</template>
...
import TexnikCreate from '../Texnik/TexnikXizmatAdd.vue'
...
const columns = [
    ...
    //                             onPositiveClick: () => {
    //                                 // axios.post('/inspection_category/delete/' + row.id)
    //                                 //     .then((res) => {
    //                                 //         getAllTexnik()
    //                                 //     })
...
const searchTexnik = () => {
    if (searchText.value.length > 0) {
        ...
        axios.post('/texnik_xizmat/search', data)
            .then(function (res) {
                if (res.success) {
                    tekshiruvList.value = res.data
                }
