<template>
    <div class="container">
        <div class="container_header">
            <div class="header_btn">
                <n-button type="info" @click="registrationAdd">
                    <n-icon size="18">
                        <PlusCircle />
                    </n-icon><span style="padding-left: 5px; font-size: 16px; font-weight: 500;">{{ $t('add_btn')
                        }}</span>
                </n-button>
            </div>
        ...
        <div class="content">
            <n-data-table :pagination="paginationReactive" :columns="columns" :data="branchList" :row-props="rowProps" striped />
        </div>
    </div>

    <n-modal v-model:show="addedModal" class="custom-card" preset="card" :style="{ width: '1000px' }" :title="branchId ? t('update') : t('add_btn')">
        <CreateVue @close="closeModal" :branchId="branchId" />
    </n-modal>
</template>
...
import { useMessage, useNotification, NButton, NIcon, NImage, useDialog } from 'naive-ui'
...
const columns = [
    ...
    // {
    //     title: t('use_time_filial'),
    //     render(row) {
    //         return dayJS(row.use_time * 1000).format('YYYY-MM-DD')
    //     }
    // },
    {
        title: '',
                    ...
                    NButton,
                    {
                        ...
                        onClick: (e) => {
                            ...
                            dialog.info({
                                ...
                                onPositiveClick: () => {
                                    axios.patch('/filial/delete/'+row.id,)
                                        .then((res) => {
                                            if (res.success) {
                                                getBranch()
                                            }
