<template>
  ...
  <n-modal
    ...
    preset="card"
    :style="{width: '1000px'}"
    :title="tekshiruBolimId ? t('update') : t('add_btn')"
  >
    <TekshiruvBolimAdd @close="closeModal" :tekshiruBolimId="tekshiruBolimId"/>
  </n-modal>
</template>
...
import TekshiruvBolimAdd from '../Tekshiruv/TekshiruvBolimAdd.vue'
...
const tekshiruBolimId = ref(null)
...
const rowProps = (row) => {
  return {
    ...
    onClick: () => {
      tekshiruBolimId.value = row.id
      addedModal.value = true;
    }
  };
}
 
const getAllInspectionCategory = () => {
...
const searchTekshiruv = () => {
  if(searchText.value.length > 0){
    ...
    axios.post('/inspection_category/search', data)
    .then(function (res) {
      tekshiruvList.value = []
      if(res.success){
