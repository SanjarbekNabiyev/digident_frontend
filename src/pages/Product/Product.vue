    ...
    <div class="container">
        <div class="container_header">
            ...
            <div class="header_search">
                <input class="search" :placeholder="t('search')" v-model="searchText" type="search" @change="searchProduct()" />
        ...
        <div class="content">
            <n-data-table :pagination="paginationReactive" :columns="columns" :data="productList" :row-props="rowProps"
                striped />
        </div>
    </div>

    <n-modal v-model:show="addedModal" class="custom-card" preset="card" :style="{ width: '1000px' }"
        :title="productId ? t('update') : t('add_btn')">
        <ProductAdd @close="closeModal" :productId="productId" />
    </n-modal>
</template>
    ...
    return [
        ...
        NButton,
        {
            ...
            dialog.info({
              ...
              onPositiveClick: () => {
                axios.post('/product/delete', row)
                .then((res)=> {
                  if (res.success === false) {
                    notification.error({
                      content: t('error'),
                      meta: t('not_delete_product'),
                      duration: 2500,
...
const searchProduct = () => {
    if (searchText.value.length > 0) {
        ...
        axios.post('/product/search_product', data)
            .then(function (res) {
                productList.value = []
                if (res.success) {
                    productList.value = res.data
