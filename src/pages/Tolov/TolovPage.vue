<template>
  ...
  <n-modal v-model:show="addedModal" class="custom-card" preset="card" :style="{ width: '1000px' }" :title="tolovId ? t('update') : t('add_btn')">
    <TolovAdd @close="closeModal" :tolovId="tolovId" />
  </n-modal>
</template>
...
const searchXodim = () => {
  if (searchText.value.length > 0) {
    ...
    axios.post('/payment/search_payment', data)
      .then(function (res) {
        tolovList.value = res.data
      })
      .catch(function (error) {
