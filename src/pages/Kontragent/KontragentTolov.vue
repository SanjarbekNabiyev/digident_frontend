<template>
    ...
    <n-modal v-model:show="addedModal" class="custom-card" preset="card" :style="{ width: '1000px' }" :title="kontragentId ? t('update') : t('add_btn')">
        <KontragentTolovAdd @close="closeModal" :kontragentId="kontragentId" />
    </n-modal>
</template>
...
import KontragentTolovAdd from '../Kontragent/KontragentTolovAdd.vue'
                    ...
                    NButton,
                    {
                        ...
                        onClick: (e) => {
                            ...
                            dialog.info({
                                ...
                                onPositiveClick: () => {
                                    axios.post('/kontragent_pay/delete', row)
                                    .then((res)=> {
                                        getKontragent()
                                    })
...
const searchXona = () => {
    if (searchText.value.length > 0) {
        ...
        axios.post('/kontragent_pay/search_kontragent', data)
            .then(function (res) {
