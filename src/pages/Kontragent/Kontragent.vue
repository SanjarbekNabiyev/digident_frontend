<template>
    <div class="container">
        ...
        <div class="content">
            <n-data-table :pagination="paginationReactive" :columns="columns" :data="kontragentList" :row-props="rowProps"
                striped />
        </div>
    </div>

    <n-modal v-model:show="addedModal" class="custom-card" preset="card" :style="{ width: '1000px' }"
        :title="kontragentId ? t('update') : t('add_btn')">
        <KontragentAdd @close="closeModal" :kontragentId="kontragentId" />
    </n-modal>
</template>
...
import KontragentAdd from '../Kontragent/KontragentAdd.vue'
                    ...
                    NButton,
                    {
                        ...
                        onClick: (e) => {
                            ...
                            dialog.info({
                                ...
                                onPositiveClick: () => {
                                    axios.post('/kontragent/delete', row)
                                    .then((res)=> {
                                        if(res.success === false){
                                            notification.error({
                                                content: t('error'),
                                                meta: t('not_delete_kont'),
                                                duration: 2500,
...
const searchXona = () => {
    if (searchText.value.length > 0) {
        ...
        axios.post('/kontragent/search_kontragent', data)
            .then(function (res) {
