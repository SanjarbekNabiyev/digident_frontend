<template>
    <div class="xodim">
        <div class="xodim_head">
            <span>{{ $t('texnik_pay') }}</span>
        </div>
        <div class="xodim_body">
            <n-form class="form" ref="formRef" :model="xodim" :rules="rules">
                <n-form-item :label="t('texnik')" path="texnik_id">
                    <n-select v-model:value="xodim.texnik_id" :options="xodimList" @update:value="chooseXodim" value-field="id" label-field="fullname" :placeholder="t('texnik')" filterable clearable/>
                </n-form-item>
                ...
                <n-form-item :label="t('comment')">
                    <n-input v-model:value="xodim.place" type="text" :placeholder="t('comment')"/>
                </n-form-item>
                <n-form-item label="">
                    <div class="btn">
...
const saveTolov = async () => {
    try {
        ...
        if(props.kontragentId){
            method = axios.patch('/texnik_pay/update/'+props.kontragentId, data)
        }else{
            method = axios.post('/texnik_pay/create', data)
        }
        method.then(function (res) {
            if(res.success){
                ...
                xodim.value.texnik_id = null
                xodim.value.paytype = 'Нақд'
                xodim.value.place = null
                xodim.value.summa = 0
                xodim.value.xaqqi = 0
...
const getOneTolov = () => {
    if(props.kontragentId){
        axios.get('/texnik_pay/info/'+props.kontragentId)
        .then(function (res) {
            let data = {...res.data}
            data['summa'] = Number(res.data.summa)

            chooseXodim(res.data.texnik_id)
            xodim.value = data
        })
        .catch(function (error) {
            console.log(error.message);
