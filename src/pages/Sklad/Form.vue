    ...
    <div class="container">
        <!-- <div class="container_header" v-if="!disabledSelect">
            <div class="header_btn">
                <n-select v-model:value="userId" :options="doctorList" filterable clearable label-field="full_name"
                    value-field="id" :placeholder="t('xodim')" size="medium" :disabled="disabledSelect"
                    style="width: 250px;" />
                <n-button type="primary" style="width: 120px;" size="medium" @click="filterDoc"
                    :disabled="disabledSelect">{{ $t('show_btn') }}</n-button>
            </div>
            <n-button type="error" style="width: 120px;" size="medium" @click="clearDoc">{{ $t('clear_btn')
                }}</n-button>
        </div> -->
        <div class="content">
...
const getAll = () => {
    axios.get('/report/reagent-sklad')
        .then(function (res) {
            if (res.success) {
