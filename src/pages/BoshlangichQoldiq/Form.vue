<template>
    <div class="container">
        <div class="container_header">
            <div class="header_btn">
                ...
                <n-button type="info" @click="showInfo">
                    <n-icon size="18">
                        <Eye />
                    </n-icon><span style="padding-left: 5px; font-size: 16px; font-weight: 500;">{{ $t('show_btn')
                        }}</span>
                </n-button> -->
            </div>
        ...
        <div class="content">
            <n-data-table :pagination="paginationReactive" :columns="columns" :data="prixodList" :row-props="rowProps" :max-height="750" striped />
        </div>
    </div>
</template>
                    ...
                    NButton,
                    {
                        ...
                        onClick: (e) => {
                            ...
                            dialog.info({
                                ...
                                onPositiveClick: () => {
                                    axios.post('/boshlangichqoldiq/delete', row)
                                        .then((res) => {
                                            getPrixod()
                                        })
...
const rowProps = (row) => {
    return {
        ...
        onClick: () => {
            router.push({ name: "BoshlangichQoldiqAdd", query: { id: row.id } })
        }
    };
}

const registrationAdd = () => {
    router.push({ name: "BoshlangichQoldiqAdd" });
}
...
// Get all prixod api 
const getPrixod = () => {
    axios.get('/boshlangichqoldiq/all')
        .then(function (res) {
            if (res.data) {
